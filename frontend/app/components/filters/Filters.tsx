...
const Filters = () => {
  ...
  const handleClick = (target: EventTarget) => {
    ...
    // Ensure only one checkbox per category is checked
    checkboxes.forEach((item) => {
      const inputElement = item as HTMLInputElement;
      if (inputElement !== checkbox) inputElement.checked = false;
    });

    // Get all selected filters
    const selectedFilters: { [key: string]: string | null } = {};

    // Check each filter category for checked inputs
    ["jobType", "education", "experience", "salary"].forEach((category) => {
      const checkedInput = document.querySelector(
        `input[name="${category}"]:checked`
      ) as HTMLInputElement;
      
      if (checkedInput) {
        selectedFilters[category] = checkedInput.value;
      } else {
    ...
    Object.keys(selectedFilters).forEach((filterKey) => {
      ...
      if (filterValue) {
        newQuery.set(filterKey, filterValue);
      }
    });
